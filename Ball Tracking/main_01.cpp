...
const char TEST_WIN[] = "Test Window";
...
int main(int argc, char** argv) {
		...
		bool connected2RS = false;							// Track whether main loop entered/connected		
																						/*Mat properties*/
		int frameRate = 60;
		...
		while (g_sessionManager->AcquireFrame(true) >= PXC_STATUS_NO_ERROR) {
				...
				if (colorImage && enableColor) {
						...
						RSColorPXCImage2Mat(sampleR->color, &frameColor); 
				...
				if (depthImage && enableDepth) {
						...
						double borderSizeX = 0.03; // 0.1 takes 10% off all edges, 0.2 takes off 20%...
						double borderSizeY = 0.03; // 0.1 takes 10% off all edges, 0.2 takes off 20%...
				...
				Mat hueROI = hueMask(rectColorROI);
				/*Find ROI around object in color image for more processing*/
				...
				int minRadius, maxRadius, minDist;
				if (cROIMorph.rows > cROIMorph.cols) {
						maxRadius = (int)cROIMorph.cols * 0.9;
						minRadius = (int)cROIMorph.cols * 0.4;
						minDist = (int)cROIMorph.cols * 0.4;
				}
				else {
						maxRadius = (int)cROIMorph.rows * 0.9;
						minRadius = (int)cROIMorph.rows * 0.4;
						minDist = (int)cROIMorph.rows * 0.4;
				}
				//HoughCircles(hueROI, circles, CV_HOUGH_GRADIENT, 1, 1, 200, 100, 0, 0);
				...
				//		circle(cROICanny, center, radius, Scalar(0, 0, 255), 1, 4, 0);
				//}
				...
				if (cROICanny.data) {
						imshow(CANNY1_WIN, cROICanny);
				}				
				if (hueROI.data) {
						imshow(TEST_WIN, hueROI);
				}
				...
				if (firstLoop) {
						...
						cout << "----\n";
						printMatProperties(&hueROI, "hueROI");
